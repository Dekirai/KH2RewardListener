...
{
    public partial class BlockInputReward : UserControl
    {
        public BlockInputReward()
        {
            InitializeComponent();
            CheckSettingsFile();
            LoadSettingsFile();
        ...
        public void LoadSettingsFile()
        {
            ...
            try
            {
                RewardName = ini.Sections["BlockInput"].Keys["RewardName"].Value;
                ChatMessage = ini.Sections["BlockInput"].Keys["ChatMessage"].Value;
                Duration = int.Parse(ini.Sections["BlockInput"].Keys["Duration"].Value);
            }
            catch
            {
                RewardName = "Block Input";
                ChatMessage = "Sora is no longer able to move for [Duration] seconds.";
                Duration = 10000;
            }
        }
        ...
        private void bt_Save_Click(object sender, EventArgs e)
        {
            ...
            var ini = new IniFile();
            ini.Load(Environment.CurrentDirectory + @"\config_rewards.ini");
            if (!ini.Sections.Contains("BlockInput"))
            {
                var section = ini.Sections.Add("BlockInput");
            ...
            {
                ini.Sections["BlockInput"].Keys["RewardName"].Value = RewardName;
                ini.Sections["BlockInput"].Keys["ChatMessage"].Value = ChatMessage;
                ini.Sections["BlockInput"].Keys["Duration"].Value = Duration.ToString();
            }
        ...
        public void DoAction()
        {
            ...
            Thread thread = new Thread(BlockInput);
            thread.Start();
        }

        private async void BlockInput()
        {
            MainForm.kh2.WriteByte(0x2A148E8, 0x01);
            await Task.Delay((int)Duration);
            MainForm.kh2.WriteByte(0x2A148E8, 0x00);

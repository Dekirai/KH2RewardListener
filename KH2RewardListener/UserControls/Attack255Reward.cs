...
{
    public partial class Attack255Reward : UserControl
    {
        public Attack255Reward()
        {
            InitializeComponent();
            CheckSettingsFile();
            LoadSettingsFile();
        ...
        public void LoadSettingsFile()
        {
            ...
            try
            {
                RewardName = ini.Sections["Attack255"].Keys["RewardName"].Value;
                ChatMessage = ini.Sections["Attack255"].Keys["ChatMessage"].Value;
                Duration = int.Parse(ini.Sections["Attack255"].Keys["Duration"].Value);
            }
            catch
            {
                RewardName = "Attack 255";
                ChatMessage = "Sora's attack has been set to 255 for [Duration] seconds.";
                Duration = 10000;
            }
        }
        ...
        private void bt_Save_Click(object sender, EventArgs e)
        {
            ...
            var ini = new IniFile();
            ini.Load(Environment.CurrentDirectory + @"\config_rewards.ini");
            if (!ini.Sections.Contains("Attack255"))
            {
                var section = ini.Sections.Add("Attack255");
            ...
            {
                ini.Sections["Attack255"].Keys["RewardName"].Value = RewardName;
                ini.Sections["Attack255"].Keys["ChatMessage"].Value = ChatMessage;
                ini.Sections["Attack255"].Keys["Duration"].Value = Duration.ToString();
            }
        ...
        public void DoAction()
        {
            ...
            Thread thread = new Thread(Attack255);
            thread.Start();
        }

        private async void Attack255()
        {
            var old = MainForm.kh2.ReadByte(0x2A20E20);
            MainForm.mem.FreezeValue($"KINGDOM HEARTS II FINAL MIX.exe+2A20E20", "byte", "0xFF");

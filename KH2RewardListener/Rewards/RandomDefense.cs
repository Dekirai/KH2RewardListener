...
namespace KH2RewardListener.Rewards
{
    public class RandomDefense
    {
        ...
        public static void DoAction()
        {
            ...
            string rewardjson = File.ReadAllText("Rewards/RandomDefense.json");
            ...
            var old = mem.ReadByte($"{process}.exe+0x2A20E24");
            ...
            new Thread(() =>
            {
                while (counter > 0)
                {
                    ...
                    mem.WriteMemory($"{process}.exe+0x2A20E24", "byte", $"0x{value.ToString("X")}");
                    Thread.Sleep(1000);
                    counter--;
                }
                mem.WriteMemory($"{process}.exe+0x2A20E24", "byte", $"0x{old.ToString("X")}");

...
namespace KH2RewardListener.Rewards
{
    public class FieldOfView
    {
        ...
        {
            GetPID();
            string rewardjson = File.ReadAllText("Rewards/FieldOfView.json");
            ...
            new Thread(() =>
            {
                while (counter > 0)
                {
                    ...
                    Hypervisor.FreezeAddress<float>(0x718D40, 3);
                    Thread.Sleep(1000);
                    counter--;
                }
                Hypervisor.UnfreezeAddress(0x718D40);
                Hypervisor.Write<float>(0x718D40, 1.5f);

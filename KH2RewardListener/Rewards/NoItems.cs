...
namespace KH2RewardListener.Rewards
{
    public class NoItems
    {
        ...
        {
            GetPID();
            string rewardjson = File.ReadAllText("Rewards/NoItems.json");
            ...
            new Thread(() =>
            {
                while (counter > 0)
                {
                    ...
                    mem.WriteMemory($"{process}.exe+2A5A126", "byte", "0x00");
                    Thread.Sleep(1000);
                    counter--;
                }
                mem.WriteMemory($"{process}.exe+2A5A126", "byte", "0x03");

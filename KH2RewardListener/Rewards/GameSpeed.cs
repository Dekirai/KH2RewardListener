...
namespace KH2RewardListener.Rewards
{
    public class GameSpeed
    {
        ...
        {
            GetPID();
            string rewardjson = File.ReadAllText("Rewards/GameSpeed.json");
            ...
            new Thread(() =>
            {
                while (counter > 0)
                {
                    ...
                    Hypervisor.FreezeAddress<float>(0x717424, 3);
                    Thread.Sleep(1000);
                    counter--;
                }
                Hypervisor.UnfreezeAddress(0x717424);
                Hypervisor.Write<float>(0x717424, 1);

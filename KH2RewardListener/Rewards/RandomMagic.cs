...
namespace KH2RewardListener.Rewards
{
    public class RandomMagic
    {
        ...
        public static void DoAction()
        {
            ...
            var old = mem.ReadByte($"{process}.exe+0x2A20E22");
            ...
            new Thread(() =>
            {
                while (counter > 0)
                {
                    ...
                    mem.WriteMemory($"{process}.exe+0x2A20E22", "byte", $"0x{value.ToString("X")}");
                    Thread.Sleep(1000);
                    counter--;
                }
                mem.WriteMemory($"{process}.exe+0x2A20E22", "byte", $"0x{old.ToString("X")}");

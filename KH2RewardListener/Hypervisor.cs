...
namespace KH2RewardListener
{
    public static class Hypervisor
    {
        ...
        //- Freeze/Unfreeze a value at a given address
        ...
        private static Dictionary<ulong, byte[]> FrozenAddresses = new();
        private static CancellationTokenSource FreezeCancellation = new();
        ...
        /// <summary>
        /// Freezes a value at a given address until UnfreezeAddress is called.
        /// </summary>
        /// <typeparam name="T">Type of the value to write. Must have a size.</typeparam>
        ...
        /// <param name="Absolute">If the address is absolute, false by default.</param>
        /// <param name="ModuleName">The name of the module to read from. Defaults to main executable.</param>
        public static void FreezeAddress<T>(ulong Address, T Value, bool Absolute = false, string? ModuleName = null) where T : struct
        {
            var _address = (IntPtr)Address;
            ...
            if (_inSize > 1)
            {
                _inArray = (byte[])typeof(BitConverter).GetMethod("GetBytes", new[] { typeof(T) })!.Invoke(null, new object[] { Value });
            }
            else
            {
                _inArray = new byte[] { (byte)Convert.ChangeType(Value, typeof(byte)) };
            }

            WriteProcessMemory(Handle, _address, _inArray, _inArray.Length, ref _inWrite);
            FrozenAddresses[Address] = _inArray; // Store the frozen value

            if (FreezeCancellation.IsCancellationRequested)
            {
                FreezeCancellation = new CancellationTokenSource();
            }

            // Start background freezing thread
            Task.Run(() =>
            {
                while (!FreezeCancellation.Token.IsCancellationRequested && FrozenAddresses.ContainsKey(Address))
                {
                    WriteProcessMemory(Handle, _address, _inArray, _inArray.Length, ref _inWrite);
                    Thread.Sleep(50); // Adjust frequency if needed
                }
            }, FreezeCancellation.Token);
        }

        /// <summary>
        /// Unfreezes a previously frozen address, allowing it to be modified again.
        /// </summary>
        /// <param name="Address">The address to unfreeze.</param>
        public static void UnfreezeAddress(ulong Address)
        {
            if (FrozenAddresses.ContainsKey(Address))
            {
                FrozenAddresses.Remove(Address);
            }

            if (FrozenAddresses.Count == 0)
            {
                FreezeCancellation.Cancel();
            }
        }
